 ...
 *
 * 注意：宿主 systemPrompt.section 的 text 回调同步执行（不 await），
...
/** systemPrompt.section 段名。 */
export const LSP_PROMPT_SECTION_NAME = 'lsp:tools';

/** 段排序：置于策略段之后，记忆快照之前（120-130 区间）。 */
export const LSP_PROMPT_SECTION_ORDER = 125;

/**
 * 安装 LSP 提示词段到 systemPrompt.section。
 *
 * 使用 section 而非 context：自定义 Agent 若关闭 runtime-context
 * （includeRuntimeContext: false / suppressRuntimeContext），context 注入会被整体移除；
 * section 是静态系统提示词段，不受该抑制影响，且 text 仍支持按 cwd 动态返回空串。
 *
...
export function installLspPrompt(ctx: DshContext): void {
  ctx.systemPrompt.section({
    name: LSP_PROMPT_SECTION_NAME,
    order: LSP_PROMPT_SECTION_ORDER,
    text: (context) => {
